/* Sanity check of the policy algorithm through a single example
 *  build a coupled chain where neighboring nodes strongly agree with each other
 *  every node has two labels
 *  the value of the first node leans towards 1
 *  (the potential functions are specified through opengm)
 */
