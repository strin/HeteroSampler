...
namespace Tagging {
  ModelTreeUA::ModelTreeUA(ptr<Corpus> corpus, const po::variables_map& vm) 
  ...
  void ModelTreeUA::run(ptr<Corpus> test_corpus) {
    auto init_simple = [&] () {
      ModelSimple simple_model(this->corpus, vm);
      simple_model.run(test_corpus, true);
  ...
  ModelAdaTree::ModelAdaTree(ptr<Corpus> corpus, const po::variables_map& vm) 
  :ModelTreeUA(corpus, vm), 
   m_c(vm["c"].as<double>()),m_Tstar(vm["Tstar"].as<double>()), 
   etaT(vm["etaT"].as<double>()) {
    // aggregate stats. 
    ptr<CorpusLiteral> corpus_literal = cast<CorpusLiteral>(this->corpus);
    auto wordent_meanent = corpus_literal->tagEntropySimple();
    wordent = get<0>(wordent_meanent);
    wordent_mean = get<1>(wordent_meanent);
    auto wordfreq_meanfreq = corpus_literal->wordFrequencies();
    wordfreq = get<0>(wordfreq_meanfreq);
    wordfreq_mean = get<1>(wordfreq_meanfreq);
    auto tag_bigram_unigram = corpus_literal->tagBigram();
