...
class GradientMaximizer implements Maximizer {
  ...
  public boolean takeStep(FunctionState func) {
    ...
    Execution.putLogRec("value", Fmt.D(func.value()));
    Execution.putLogRec("gradientNormSquared", Fmt.D(NumUtils.l2NormSquared(gradient)));
...
class BacktrackingLineSearch {
  ...
  double stepSize;
  StatFig numReductionsFig = new StatFig();
  Options opts;
  ...
  public boolean maximize(FunctionState func, double[] dx) {
    ...
    begin_track("BacktrackingLineSearch(stepSize=%s)", Fmt.D(stepSize));
    double[] x = func.point();
