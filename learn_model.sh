...
if [ $1 == "NER" ]; then 
  factorL=$4
  cmd="./pos --inference $2 --T 8 --B 5 --train data/eng_ner/train --test data/eng_ner/test --eta 1 \
    --depthL 2 --windowL "$3" --factorL "$factorL" --output model/ner_gibbs_w"$3"_d2_f"$factorL".model --scoring NER --Q 5"
  echo $cmd
  ($cmd) > result/ner_gibbs_w$3_d2_f$factorL.xml & 
elif [ $1 == "POS_NER" ]; then 
  ...
  do
    cmd="./ocr --inference $2 --T 8 --B 5 --train data/ocr/train0 --test data/ocr/test0 --eta 1 --factorL $factorL  --output model/$path/ocr_f$factorL.model --scoring Acc --Q 3 " 
    echo $cmd
    mkdir -p model/$path
    mkdir -p result/$path
